...
module.exports = (opts, ctx) => {
  return {
    ...
    async ready() {
      ...
      const begin = async (_repoUrl, _prefix) => {
        let { repoId, repoUrl, authToken, source } = opts

        repoUrl = _repoUrl || repoUrl;
        if (repoId) {
          assert(
        ...
        let sidebar = getSidebarByToc(toc.data)

        if (_prefix) {
          sidebar = JSON.parse(JSON.stringify(sidebar).replace(/"\//g, `"${_prefix}/`));
        }
        ...
        for (const page of toc.data) {
          ...
          const permalink = _prefix ? `${_prefix}/${slug}.html` : `/${slug}.html`
        ...
        if (opts.multipleLanguage && !_prefix) {
          for (let prefix of Object.keys(opts.multipleLanguage)) {
            await begin(opts.multipleLanguage[prefix], prefix);
          }
        }
      }
      return begin();
